...
def parse_create(sql):
    ...
    infos = []                                              
    infos.append({
        'joininfos' :_getjoins(dict[0]['stmt']['fromClause'][0]),
#       'groups' : [[x['fields'][0],str(x['fields'][0])] for x in dict[0]['stmt']['groupClause']],
        'groups' : _getgroups(dict),
        'columns': [[expr_to_str(x['val']),_getalias(x)] for x in dict[0]['stmt']['targetList']],
        'where_clause': _getwhere(dict),
        'having_clause': _gethaving(dict),
        })
    return infos
...
def expr_to_str(expr):
    '''
    This function tramsforms expressions into strings.
    
    Example input:
    {'TYPE': 'AEXPR',
    'lexpr': {'TYPE': 'COLUMNREF', 'fields': ['test'], 'location': 80},
    'location': 84,
    'name': ['>='],
    'rexpr': {'TYPE': 'COLUMNREF', 'fields': ['from'], 'location': 86}}
    Exmpale output:
    'test>=from'
    '''
...
#           else:
#               return expr['funcname'][0]+'('+expr['args'][0]['fields'][0]+')'
...
def _getalias(info):
    '''
    This function returns any aliases for a selected column
    '''
    if info['name'] != None:
        return info['name'][1]
    else:
...
def _getgroups(sql):
    ...
    if sql[0]['stmt']['groupClause'] != None:
        return [[x['fields'][0],str(x['fields'][0])] for x in sql[0]['stmt']['groupClause']]
    else:
        return ''
def _getjoins(info):
    '''
...
#       table_name = str(info['larg']['relname'][1])
#       if info['alias']!= None:
#           alias = str(info['alias']['TYPE']['aliasname'][1])
#       else:
#           alias = ''
#       join_type = _gettype(info)
...
#       print('larg is', larg)
#       print('rarg is', rarg)
#       larg[0]['join_type'] = _gettype(info)
        rarg[0]['join_type'] = _gettype(info)
        if info['usingClause']!= None:
#           larg[0]['condition'] = 'using('+str(info['usingClause'])+')'
            rarg[0]['condition'] = 'using('+str(info['usingClause'][0])+')'
        else:
#           larg[0]['condition'] = 'on'+expr_to_str(info['quals'])
            rarg[0]['condition'] = 'on'+expr_to_str(info['quals'])
        join_infos.append(larg)
        join_infos.append(rarg)
        return join_infos
#       join_infos.append(_getjoins(info['larg']))
#       join_infos.append(_getjoins(info['rarg']))
#       return join_infos
       
def _getwhere(sql):
    if sql[0]['stmt']['whereClause'] == None:
        return ''
    else:
        return expr_to_str(sql[0]['stmt']['whereClause'])

def _gethaving(sql):
    if sql[0]['stmt']['havingClause'] == None:
        return ''
    else:
        return expr_to_str(sql[0]['stmt']['havingClause'])
